...
export default function OtpVerification() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let url = "";
      if (type === "userOtp") {
        url = `${API_URL}/verify-otp/${id}`;
      } else if (type === "NewEmail") {
        url = `${API_URL}/verifyUserEmail/${id}`;
      } else {
      ...
      if(response.status==200){
        showSuccessToast("Sucessfully Verify Otp");
        navigate("/login");
      }
    } catch (err) {
      console.log(response,'byee')
      if(err.response.data?.msg=='Otp Already verify'){

        navigate("/login");
      }
      else{ showErrorToast(err.response?.data?.msg || "Failed to verify OTP");
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleResendOTP = async () => {
    if (!canResend || !id) return;
  
    try {
      setCanResend(false);
      setTimeLeft(30);
  
      const response = await axios.get(`${API_URL}/resendUserOtp/${id}`);
